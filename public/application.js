...
var onUpdateGame = function onUpdateGame(crntIndex, crntValue, over) {
  event.preventDefault();
  var data = {
    'game': {
      'cell': {
  ...
  api.updateGame(JSON.stringify(data)).then(ui.updateGameSuccess).catch(ui.updateGameFailure);
};
...
var updateGame = function updateGame(data) {
  ...
  var apiUrl = config.apiOrigin + '/games/' + store.game.id;
  var tkn = store.user.token;
  console.log(tkn);
...
var authEvents = __webpack_require__(3);
var turn = ['X', 'O', 'X', 'O', 'X', 'O', 'X', 'O', 'X'];
var crntBoard = ['', '', '', '', '', '', '', '', ''];
var i = 0;
var player = void 0;
var notPlayer = void 0;
var over = false;

var gameWon = function gameWon(winner) {
  over = true;
...
var playerGo = function playerGo() {
  event.preventDefault();
  $(this).text(turn[i]);
  crntBoard[i] = turn[i];
  var crntValue = turn[i];
  var crntIndex = i;
